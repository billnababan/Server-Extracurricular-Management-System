...
export const getMe = async (req, res) => {
  const { uuid } = req.user;
  try {
    const [data] = await query(`SELECT uuid, username, kelas, photo, is_admin FROM users WHERE uuid = ?`, [uuid]);
    return res.status(200).json(data);
  } catch (error) {
    return res.status(400).json({ success: false, msg: error.message });
  }
...
export const ubahRole = async (req, res) => {
  const { id } = req.params;
  try {
    const cekRole = await query(`SELECT is_admin FROM users WHERE uuid = ?;`, [id]);

    if (cekRole[0].is_admin == 1) {
      await query(`UPDATE users SET is_admin = ? WHERE uuid = ?;`, [0, id]);
      return res.status(201).json({ success: true, msg: "Role Updated Siswa!" });
    } else if (cekRole[0].is_admin == 0) {
      await query(`UPDATE users SET is_admin = ? WHERE uuid = ?;`, [1, id]);
      return res.status(201).json({ success: true, msg: "Role Updated Admin!" });
    }
  } catch (error) {
    return res.status(400).json({ success: false, msg: error.message });
  }
...
export const updateUser = async (req, res) => {
  ...
  try {
    if (req.file === undefined) {
      await query(`UPDATE users SET username = ?, kelas = ? WHERE uuid = ?;`, [username, kelas, uuid]);
    } else {
      ...
      await query(`UPDATE users SET username = ?, kelas = ?, photo = ? WHERE uuid = ?;`, [username, kelas, photo, uuid]);
    }
...
export const resetPassword = async (req, res) => {
  ...
  try {
    if (password !== confpassword) {
      return res.status(400).json({ msg: "Password baru dan konfirmasi password tidak sesuai" });
    }

    const salt = await bcrypt.genSalt();
    const hashPassword = await bcrypt.hash(password, salt);

    await query(`UPDATE users SET password = ? WHERE uuid = ?;`, [hashPassword, uuid]);
