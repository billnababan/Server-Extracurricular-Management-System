...
export const addEkskul = async (req, res) => {
  ...
  try {
    ...
    const isEkskulExist = await query(`SELECT * FROM ekskul WHERE nama = ? AND is_deleted = 0;`, [nama]);
    if (isEkskulExist.length > 0) {
      return res.status(400).json({ success: false, msg: "Ekskul already exist" });
    }
...
export const updateEkskul = async (req, res) => {
  ...
  try {
    ...
    await query(`UPDATE ekskul SET nama = ?,  deskripsi = ?, jadwal = ? WHERE uuid = ?;`, [nama, deskripsi, jadwal, id]);
...
export const statusPendaftaran = async (req, res) => {
  const { id } = req.params;
  try {
    const [data] = await query(`SELECT status FROM ekskul WHERE uuid = ?;`, [id]);

    if (data.status == "Open Recruitment") {
      await query(`UPDATE ekskul SET status = "Close Recruitment" WHERE uuid = ?;`, [id]);
      return res.status(200).json("Ekskul berhasil di tutup");
    } else if (data.status == "Close Recruitment") {
      await query(`UPDATE ekskul SET status = "Open Recruitment" WHERE uuid = ?;`, [id]);
